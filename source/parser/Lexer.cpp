...
std::expected<Token, Lexer::Error> Lexer::next_token() {
    ...
    switch (const char c = stream.advance(); c) {
        ...
        case '!':
            return make_token(stream.match('=') ? Token::Type::BANG_EQUAL : Token::Type::BANG);
        case '+':
            return make_token(stream.match('=') ? Token::Type::PLUS_EQUAL : Token::Type::PLUS);
        case '-':
            return make_token(stream.match('=') ? Token::Type::MINUS_EQUAL : Token::Type::MINUS);
        case '*':
            return make_token(stream.match('=') ? Token::Type::STAR_EQUAL : Token::Type::STAR);
        case '%':
            return make_token(stream.match('=') ? Token::Type::PERCENT_EQUAL : Token::Type::PERCENT);
        case '^':
            return make_token(stream.match('=') ? Token::Type::CARET_EQUAL : Token::Type::CARET);
        case '=':
            return make_token(stream.match('=') ? Token::Type::EQUAL_EQUAL : Token::Type::EQUAL);
        case '&': {
            if (stream.match('&')) {
        ...
        case '>': {
            if (stream.match('>')) {
                return make_token(stream.match('=')
                                      ? Token::Type::GREATER_GREATER_EQUAL
